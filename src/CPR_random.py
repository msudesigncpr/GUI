...
def randomize(valid_colony_dir):
    ...
    P1coloniesList = [
        "P1"
    ]  # initialize with first index being a name to compare to later on (remeber for lengths to sub 1)
    P2coloniesList = ["P2"]
    P3coloniesList = ["P3"]
    P4coloniesList = ["P4"]
    P5coloniesList = ["P5"]
    ...
    if count >= 1:
        with open("P1colonies.txt") as f:
            for line in f:
                ...
                P1coloniesList.append(
                    tempList.copy()
                )  # Append a copy of tempList to P1coloniesList
                tempList.clear()
