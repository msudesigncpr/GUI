...
def pinhole(
    ...
    row_deviation_threshold=0.1,
    column_deviation_threshold=0.1,
    ...
    # Cropping the image
    cropped_image = img[
        int(cropped_image_y - cropped_image_height) : int(
            cropped_image_y + cropped_image_height
        ),
        int(cropped_image_x - cropped_image_width) : int(
            cropped_image_x + cropped_image_width
        ),
    ]
    ...
    column_deviation = (0.5 - average_column_position) * cropped_image.shape[
        1
    ]  # deviation in pixels in the width direction
    row_deviation = (0.5 - average_row_position) * cropped_image.shape[
        0
    ]  # deviation in pixels in the height direction

    # Defining line start and end points
    vertical_line_start_point = (
        int(average_column_position * cropped_image.shape[1]),
        0,
    )
    vertical_line_end_point = (
        int(average_column_position * cropped_image.shape[1]),
    ...
    horizontal_line_start_point = (
        0,
        int(average_row_position * cropped_image.shape[0]),
    )
    horizontal_line_end_point = (
    ...
    print(
        "Column Deviation Magnitude:",
        column_deviation,
        "Row Deviation Magnitude:",
        row_deviation,
    )
    print(
        "Column Deviation Threshold:",
        column_deviation_threshold,
        "Row Deviation Threshold:",
        row_deviation_threshold,
    )

    # put text on image
    cv2.putText(
        cropped_image,
        ("(" + str(average_column_position)[:8] + ","),
        (int(0.1 * cropped_image.shape[0]), int(0.1 * cropped_image.shape[1])),
    ...
    cv2.putText(
        cropped_image,
        (str(average_row_position)[:8] + ")"),
        (int(0.1 * cropped_image.shape[0]), int(0.2 * cropped_image.shape[1])),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.5,
        (255, 255, 255),
        1,
    ...
    cv2.rectangle(
        ...
        (
            int(
                0.5 * cropped_image.shape[1]
                - column_deviation_threshold * cropped_image.shape[1]
            ),
            int(
                0.5 * cropped_image.shape[0]
                - row_deviation_threshold * cropped_image.shape[0]
            ),
        ),
        (
            int(
...
def move_YOLO_stuff(destination_folder_path):
    try:
        ...
        for file in os.listdir("./runs/detect"):
            os.rename(
                os.path.join("./runs/detect", file),
                os.path.join(destination_folder_path, file),
            )
...
def resize_images(image_folder_path):
    try:
        ...
        for image in os.listdir(image_folder_path):
            img = cv2.imread(
                os.path.join(image_folder_path, image), cv2.IMREAD_GRAYSCALE
            )
...
def process_petri_dish_image(
    image_folder_path,
    good_colony_coord_output_path,
    minimum_colony_distance=0.03,
    model_sensitivity=0.1,
    raw_yolo_dump_path="./yolo_dump",
):
    try:
        ...
        for image in os.listdir(resized_image_folder_path):
            model.predict(
                os.path.join(resized_image_folder_path, image),
                conf=model_sensitivity,
                save=True,
                imgsz=640,
                save_txt=True,
                classes=None,
        ...
        for label in os.listdir(label_folder_path):
            ...
            discriminate(
                image_file_path=os.path.join(
                    image_folder_path, base_file_name + ".jpg"
                ),
                prediction_file_path=os.path.join(label_folder_path, label),
                good_output_path=good_colony_coord_output_path,
                min_distance=minimum_colony_distance,
            )
...
def create_metadata(
    ...
    try:
        if not create_colony_view and not create_petri_dish_view:
            print(
                "No metadata created. Please specify whether to create a petri dish view, a colony view, or both. Dumb idiot."
            )
            return
        ...
        petri_dish_view_output_path = os.path.join(
            metadata_output_path, "petri_dish_view"
        )

        if not os.path.exists(colony_view_output_path) and create_colony_view:
            os.makedirs(colony_view_output_path)
            print(
                "Creating colony view metadata and putting it in: "
                + colony_view_output_path
            )

        if not os.path.exists(petri_dish_view_output_path) and create_petri_dish_view:
            os.makedirs(petri_dish_view_output_path)
            print(
                "Creating petri dish view metadata and putting it in: "
                + petri_dish_view_output_path
            )
        ...
        for image_file in os.listdir(image_folder_path):
            ...
            with open(
                os.path.join(colony_coords_folder_path, base_file_name + ".txt")
            ) as file:
                ...
                for colony_line in colony_lines:
                    ...
                    colony_number = str(
                        elements[6]
                    )  # SARAH: append the colony number (well letter/number) to the end of every line.
                    ...
                    if create_colony_view:
                        ...
                        cropped_image = image[
                            int(y - h) : int(y + h), int(x - w) : int(x + w)
                        ]
                        # random number
                        ...
                        cv2.imwrite(
                            os.path.join(bing, "_" + str(colony_number) + ".jpg"),
                            cropped_image,
                        )
                ...
                if create_petri_dish_view:
                    ...
                    cv2.imwrite(
                        os.path.join(
                            petri_dish_view_output_path, base_file_name + ".jpg"
                        ),
                        image,
                    )
...
def distance(x0, y0, r0=0, x1=0.5, y1=0.5, r1=0):
    x_dist = abs(x0 - x1) ** 2
    y_dist = abs(y0 - y1) ** 2
...
def binary_disciminate(
    ...
    try:
        ...
        # -----------------------------------------------CROP & GRAY---------------------------------
        cropped_image = img[
            int(y - height) : int(y + height), int(x - width) : int(x + width)
        ]
        ...
        binary_image = cv2.threshold(
            gray_cropped_image, threshold, 255, cv2.THRESH_BINARY
        )[1]
        ...
        average_row_position = np.dot(row_positions, row_sums) / (
            total_row_sum * width * 2
        )
        average_column_position = np.dot(column_positions, column_sums) / (
            total_column_sum * height * 2
        )

        MAX_ROW_OFFSET = 0.4
        MAX_COLUMN_OFFSET = 0.4
        ...
        for l in range(len(erosion_iterations)):
            ...
            eroded_binary_image = cv2.erode(
                binary_image, np.ones((2, 2), np.uint8), iterations=iterations
            )
            binary_image_sum = np.sum(
                eroded_binary_image
            )  # sum of all pixels in the image
            binary_image_height = binary_image.shape[0]
            binary_image_width = binary_image.shape[1]
            binary_image_sum = binary_image_sum / (
                binary_image_width * binary_image_height
            )  # normalize
            if original_display:
                title = (
                    "Erosion iteration: "
                    ...
                    + str(int(binary_image_sum))
                    + " Threshold: "
                    + str(erosion_threshold)
                )
            ...
            if (
                (binary_image_sum > erosion_threshold)
                or (row_offset > MAX_ROW_OFFSET)
                or (column_offset > MAX_COLUMN_OFFSET)
            ):
                ...
                if bad_display:
                    title = (
                        ...
                        + str(erosion_threshold)
                        + "_col_"
                        + str(average_column_position)
                        + "_row_"
                        + str(average_row_position)
                    )  # random shit is for testing
                    cv2.circle(
                        img, (int(x), int(y)), int(width / margin), (0, 0, 255), 1
                    )
                    img = cv2.resize(img, (640, 640))
                    ...
                    if save_folder_path is not None:
                        ...
                        eroded_binary_image = cv2.resize(
                            eroded_binary_image, (640, 640)
                        )
                        file_name = os.path.splitext(os.path.basename(img_file_path))[0]
                        binary_save_path = os.path.join(
                            save_path, file_name + "_" + title + ".jpg"
                        )
                        cropped_save_path = os.path.join(
                            save_path,
                            random_lil_fucker + "cropped" + file_name + ".jpg",
                        )
...
def discriminate(
    ...
    try:
        if good_output_path is None and bad_output_path is None:
            print(
                "Error: Please specify the output paths for the good or bad colony location files"
            )
            exit()
        ...
        if good_output_path is not None:
            ...
            good_file_name = os.path.join(
                str(good_output_path), base_file_name + ".txt"
            )
        ...
        with open(prediction_file_path) as predictionFile:
            ...
            for main_colony_line in lines:
                ...
                ratio = abs(
                    (float(main_colony[4]) / float(main_colony[3])) - 1
                )  # ok really this is how not square it is not the ratio but close enough

                is_bad_colony = True
                if (
                    distance(x0=main_colony_x, y0=main_colony_y) < petri_dish_radius
                    and main_colony_confidence  # discriminate against colonies that are outside / near edge or petri dish
                    > min_selection_confidence
                    and binary_disciminate(  # discriminate against colonies that are not confident enough
                        img_file_path=image_file_path,
                        x=main_colony_x,
                        y=main_colony_y,
                        width=main_colony_w,
                        height=main_colony_h,
                        original_display=binary_original_display,
                        good_display=binary_good_display,
                        bad_display=binary_bad_display,
                        display_time=display_time,
                        margin=binary_discrimination_margin,
                        save_folder_path=binary_save_folder_path,
                    )
                ):  # discriminate against colonies that have too much shit near them
                    is_bad_colony = False
                    ...
                    for neighbor_colony_line in lines:
                        ...
                        distance_between_colonies = distance(
                            x0=main_colony_x,
                            y0=main_colony_y,
                            r0=main_colony_w,
                            x1=neighbor_colony_x,
                            y1=neighbor_colony_y,
                            r1=neighbor_colony_r,
                        )

                        if (
                            distance_between_colonies < min_distance
                            and distance_between_colonies != 0.0  # distance to colony
                            and neighbor_colony_confidence  # make sure it's not the same colony
                            > min_discrimination_confidence
                        ):  # make sure the colony prediction is confident enough to be used for discrimination
                            is_bad_colony = True
                ...
                if is_bad_colony and bad_output_path is not None:
                    ...
                    if not good_colonies.__contains__(main_colony_line):
                        with open(good_file_name, "a") as good_file:
                            ...
                            lines.remove(
                                main_colony_line
                            )  # if the colony is good (for the most part this just means isolated),
...
def showPredictions(
    good_colony_file_path=None,
    bad_colony_file_path=None,
    image_path=None,
    display_image=False,
    display_time=2000,
    save_folder_path=None,
    ...
    # GSD height: 0.179
    # GSD width: 0.142
    box_width = int(0.4 / 0.142)
    box_height = int(0.4 / 0.179)
        ...
        with open(good_colony_file_path) as good_colony_file:
            ...
            for colony_line in good_colonies:
                if colony_line is not None:
                    ...
                    cv2.rectangle(
                        image,
                        (int(x - box_width / 2), int(y - box_height / 2)),
                        (int(x + box_width / 2), int(y + box_height / 2)),
                        (0, 0, 255),
                        1,
                    )
    ...
    try:
        ...
        with open(prediction_file_path) as file:
            ...
            for colony_line in colony_lines:
                ...
                box_width = int(1 / 0.106)
                box_height = int(1 / 0.134)
                ...
                cv2.circle(img, (x, y), int(0.03 * img_width / 2), (0, 255, 0), 1)
