...
def main():
    ...
    os.makedirs(pinHole)
    asyncio.run(
        CPRmotorctrl.pinhole(drive_ctrl, pinHole)
    ...
    asyncio.run(
        CPRmotorctrl.takePhotos(imagesforProcessingFolder, numPetriDishes, drive_ctrl)
    )
    ...
    cpr.process_petri_dish_image(
        image_folder_path=imagesforProcessingFolder,
        good_colony_coord_output_path=goodColoniesFolder,
    )
